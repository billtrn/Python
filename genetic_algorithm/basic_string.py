...
def basic(target: str, genes: list[str], debug: bool = True) -> tuple[int, int, str]:
    ...
    for _ in range(N_POPULATION):
        population.append("".join([secrets.SystemRandom().choice(genes) for i in range(len(target))]))
    ...
    while True:
        ...
        def mutate(child: str) -> str:
            ...
            if secrets.SystemRandom().uniform(0, 1) < MUTATION_PROBABILITY:
                child_list[secrets.SystemRandom().randint(0, len(child)) - 1] = secrets.SystemRandom().choice(genes)
